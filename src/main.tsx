...
export default function App() {
  ...
  const load = async (targetPage = page, targetSearch = search) => { try { const result = await invoke<Order[]>('list_orders', { page: targetPage, page_size: PAGE_SIZE, search: targetSearch || null }); setOrders(result); setTotal(targetSearch ? result.length : await invoke<number>('count_orders')); } catch (e) { setError(String(e)); } };
  const sync = async () => { if (busyRef.current) return; busyRef.current = true; setBusy(true); setError(null); try { await invoke('sync_recent_orders'); setPage(1); await load(1); } catch (e) { setError(String(e)); } finally { busyRef.current = false; setBusy(false); setNextSyncAt(Date.now() + SYNC_INTERVAL_MS); } };
